...
|-----------|----------------------|---------------------|---------------|
| [[a0-existential-difference.axiom\|A0]]+[[t1-asymmetry.theorem\|T1]] (Existential Difference + Asymmetry) | [[phi-golden-ratio.constant\|φ — Golden Ratio (constant)]] (golden ratio) | [[c-difference-propagation-limit.boundary\|c — Difference Propagation Limit (boundary)]] (propagation limit) | Optimal proportion + maximum speed |
| [[a0-existential-difference.axiom\|A0]]+[[t2-irreducibility.theorem\|T2]] (Existential Difference + Irreducibility) | [[e-eulers-number.constant\|e — Euler's Number (constant)]] (natural growth) | [[epsilon-difference-resolution-quantum.boundary\|ε — Difference Resolution Quantum (boundary)]] (resolution quantum) | Growth rate + minimum difference |
...
### **A0+T1 (Existential Difference + Asymmetry) → (φ, c)**

#### **Reasoning Chain to φ (Golden Ratio)**

**Logical Necessity:**
1. **A0+T1 establishes**: Difference must exist distinguishably and is inherently directional — ∆(A→B) ≠ ∆(B→A)
...
**Logical Necessity:**
1. **A0+T1 establishes**: Difference propagation must maintain distinguishable existence and is directional and causal
...
### **A0+T2 (Existential Difference + Irreducibility) → (e, ε)**

#### **Reasoning Chain to e (Natural Growth Constant)**

**Logical Necessity:**
1. **A0+T2 establishes**: Primitive difference (∆) must exist distinguishably and cannot be decomposed without loss of distinction
...
**Logical Necessity:**
1. **A0+T2 establishes**: Difference cannot be decomposed below a certain level without loss of distinguishable existence
...
|------------------|----------------------------|----------------------|
| A0+T1 → φ | Golden ratio algorithms | `(1 + sqrt(5))/2`, Fibonacci limits |
| A0+T1 → c | Speed-of-light constants | `299792458 m/s`, causal limits |
| A0+T2 → e | Exponential growth functions | `Math.exp()`, compound interest |
| A0+T2 → ε | Floating-point precision limits | `Number.EPSILON`, quantum thresholds |
...
|------------------|-------------------|---------|
| A0+T1 → φ | Pleasing proportions | "What feels naturally balanced" |
| A0+T1 → c | Information travel limits | "How fast news can spread" |
| A0+T2 → e | Organic growth patterns | "How life naturally grows" |
| A0+T2 → ε | Just-noticeable differences | "Smallest change you can feel" |
...
- [[t1-asymmetry.theorem\|T1 — Asymmetry (theorem)]] - Source of φ and c derivation (with A0)
- [[t2-irreducibility.theorem\|T2 — Irreducibility (theorem)]] - Source of e and ε derivation (with A0)
