...
As the inward spiral tightens, each Architrino gets progressively closer to its own recent path. This proximity intensifies the self-repulsive force, which counteracts the inward pull from the partner. Eventually, a state of equilibrium is reached where the attractive force is perfectly balanced by the self-repulsive force that prevents the path from curving any further.

At this point, the inward spiral halts, and the system settles into a **stable, circular orbit of a minimum radius**. This final state is a dynamic equilibrium.
