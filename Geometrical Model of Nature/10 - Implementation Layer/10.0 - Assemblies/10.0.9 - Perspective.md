...
- What we assume:
  ...
  - Each causal hit is strictly radial: the acceleration contribution lies along $\hat{\mathbf{r}}$ from the retarded source point to the receiver.
...
- With $\delta\to\delta_\varepsilon$, compute the mean inward attraction from the partner versus the mean outward self-repulsion across one orbit; the fixed point defines $d_0$ and the maximum orbital frequency $2\pi/t_0$.
