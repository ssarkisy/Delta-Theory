...
- What we assume:
  - Emission cadence is constant; the effective per-wavefront amplitude is $q_{\mathrm{eff}}(t_0)=\dfrac{q}{1+|v_{\text{source}}(t_0)|}$.
  - Receiver response is likewise modulated by $1/(1+|v_{\text{receiver}}(t)|)$.
  - Canonical per-hit law:
    $$
    a_{o′\leftarrow o} \propto \sigma_{q_o q_{o′}}\,\frac{|q_o q_{o′}|}{r^2\,\big(1+|v_o|\big)\,\big(1+|v_{o′}|\big)}\,\hat{\mathbf{r}}.
    $$
...
- What we assume:
  ...
  - We use a narrow Gaussian shell $\delta_\varepsilon$ when continuous-time derivatives are needed.
...
- With $\delta\to\delta_\varepsilon$, compute the mean inward attraction from the partner versus the mean outward self-repulsion across one orbit; the fixed point defines d0 and the maximum orbital frequency $2\pi/t_0$.
