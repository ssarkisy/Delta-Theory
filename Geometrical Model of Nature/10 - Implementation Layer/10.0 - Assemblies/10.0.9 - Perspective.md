...
## 2) Constant per-wavefront emission (no dimming)

- What we assume:
  - Emission cadence and per-wavefront amplitude are constant.

- Why it matters:
  - Simplifies calibration and emphasizes that stability and scale selection arise from delay and self-interaction, not from amplitude dimming. Receiver motion influences only instantaneous power via $\mathbf{F}\cdot\mathbf{v}$ through the radial component $v_r$.
...
- Distance–speed trade: A ∝ f(|v_em|)/r² leaves r and |v_em| entangled.
