...
- What we assume:
  - Emission cadence is constant; the effective per-wavefront amplitude is $q_{\mathrm{eff}}(t_0)=q\,f\!\big(\beta_{\text{source}}(t_0)\big)$ with $\beta_{\text{source}}=\|\mathbf{v}_{\text{source}}(t_0)\|/v$ (default $f(\beta)=1/(1+\beta)$).
  - Canonical per-hit law:
    $$
    a_{o′\leftarrow o} \propto \sigma_{q_o q_{o′}}\,\frac{|q_o q_{o′}|}{r^2}\,f\!\big(\|\mathbf{v}_o\|\big)\,\hat{\mathbf{r}}.
    $$
