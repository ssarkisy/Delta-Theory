...
- Source impulse at $(t_0,\mathbf{s}_0)$ creates a shell supported on $r = v(t-t_0)$ with surface density that conserves a constant per-shell amplitude $q$ (no speed-based dimming):
  $$
...
- This solves the radial continuity (transport) equation
  ...
  $$
- Emission is continuous with constant time-density $q(t)\equiv q_0$ (no explicit emitter-speed dimming).
...
- Pros
  - Directly implements the project’s delayed, radial-only interaction law with constant emission cadence and no explicit emitter-speed dimming.
