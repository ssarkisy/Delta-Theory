...
At cosmological scales we assume the total Architrino charge inventory sums to zero (equal counts of $+\epsilon$ and $-\epsilon$). This serves as a boundary condition in continuum arguments and avoids unphysical large-scale divergences. Local subsystems can be non-neutral; neutrality is imposed only as a global working hypothesis.
