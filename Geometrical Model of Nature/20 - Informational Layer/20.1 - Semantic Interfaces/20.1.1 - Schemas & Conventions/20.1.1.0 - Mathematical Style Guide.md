...
- Emission cadence and per-wavefront amplitude are constant (no emitter-speed dimming). Receiver velocity affects only instantaneous power $\,\mathbf{F}\cdot\mathbf{v} = |\mathbf{F}|\,v_r$.
...
- Phrase velocity effects without emitter-speed dimming; emission cadence and per-wavefront amplitude are constant.
...
- Notation lint (common mistakes):
  ...
  - Reserve $v$ for the field speed (scalar). Use $\|\mathbf{v}\|$ for speed magnitude, or $\beta=\|\mathbf{v}\|/v$; with $v=1$, $\beta=\|\mathbf{v}\|$.
  - Do not introduce emitter-speed dimming functions $f(\beta)$; emission cadence and per-wavefront amplitude are constant.
