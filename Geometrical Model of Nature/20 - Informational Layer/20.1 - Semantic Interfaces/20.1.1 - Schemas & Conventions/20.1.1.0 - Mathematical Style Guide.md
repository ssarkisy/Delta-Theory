...
- $q_{\mathrm{eff}}(t_0) = q\,f\!\big(\lvert v_{\text{source}}(t_0)\rvert\big)$ (per-wavefront amplitude; emission cadence is constant; default $f(|v|)=1/(1+|v|)$). Receiver velocity affects only instantaneous power $\,\mathbf{F}\cdot\mathbf{v} = |\mathbf{F}|\,v_r$.
