...
- $q_{\mathrm{eff}}(t_0) = q\,f\!\big(\beta_{\text{source}}(t_0)\big)$ where $\beta_{\text{source}}(t_0)=\|\mathbf{v}_{\text{source}}(t_0)\|/v$ (in $v=1$ units, $\beta=\|\mathbf{v}\|$). Default $f(\beta)=1/(1+\beta)$. Receiver velocity affects only instantaneous power $\,\mathbf{F}\cdot\mathbf{v} = |\mathbf{F}|\,v_r$.
